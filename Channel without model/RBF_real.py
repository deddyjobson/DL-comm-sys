...
parser.add_argument('--lr_t',type=float,default=3e-5) # learning rate
parser.add_argument('--lr_r',type=float,default=3e-5) # learning rate
...
# make network
if hp.load_best:
    encoder = torch.load( join('Best','best_encoder_RBF_real({1},{2})_{0}.pt'.format(hp.SNR,hp.n,hp.k)) )
    dec_params = torch.load( join('Best','best_decoder_RBF_real({1},{2})_{0}.pt'.format(hp.SNR,hp.n,hp.k)) ).to(device)
    def decoder(x):
        y = decoder.lin1(x)
        y = torch.tanh(y)
        y = decoder.lin2(y)
...
def receiver_step(): # supervised learning
    ...
    for p in dec_params.parameters():
        if torch.isnan(p.grad).any():
            print('Nan gradients, aborting training.')
            return 0
    optim_dec.step()
    return 1
    ...
    for t in range(1,hp.n_epochs):
        for _ in range(hp.n_batches_r):
            if not receiver_step():
                return # halt training immediately
            # receiver_step()
            pass
...
if not hp.load_best:
    ...
    try:
        best_acc = np.loadtxt(join('Best','best_acc_RBF_real({1},{2})_{0:.2f}.txt').format(hp.SNR,hp.n,hp.k))
    except OSError:
        best_acc = 0

    if candidate > best_acc:
        print('New best accuracy!')
        np.savetxt(join('Best','best_acc_RBF_real({1},{2})_{0:.2f}.txt').format(hp.SNR,hp.n,hp.k) , np.array([candidate]))
        copyfile('RBF_real.py', join('Best','best_RBF_real({1},{2})_{0}.py'.format(hp.SNR,hp.n,hp.k)) )
        torch.save(encoder, join('Best','best_encoder_RBF_real({1},{2})_{0}.pt'.format(hp.SNR,hp.n,hp.k)))
        torch.save(dec_params, join('Best','best_decoder_RBF_real({1},{2})_{0}.pt'.format(hp.SNR,hp.n,hp.k)))
    else:
...
if hp.plot and hp.train:
    ...
    plt.grid()
    plt.savefig( join('Training','RBF_real_({0},{1})_{2}.png'.format(hp.n,hp.k,hp.SNR_dB)) )
    plt.show()
