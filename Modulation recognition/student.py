...
parser.add_argument('--pc',type=int,default=9278475) # parameter count of teacher model
# parser.add_argument('--rel',type=float,default=0.5) # train_test_split: train/test
parser.add_argument('--T',type=float,default=0.5) # temperature for distillation
...
# Y_train = np.array(list(map(lambda x: mods.index(lbl[x][0]), train_idx))).reshape(-1,1)
# Y_test = np.array(list(map(lambda x: mods.index(lbl[x][0]), test_idx))).reshape(-1,1)
...
teacher = models.load_model( os.path.join('Best','teacher_model{0}.h5'.format(hp.pc)) )
...
# Y_test = teacher.predict(X_test)

#no need to modify true test labels
Y_train = Y_train ** (1/hp.T)
...
filename = os.path.join('Models','student_{0}.wts.h5'.format(model.count_params()))
...
try:
    ...
    if e.errno != errno.EEXIST:
        raise
try:
    best_acc = np.loadtxt(join('Best','stud_acc({0}).txt').format(model.count_params()))
...
if candidate > best_acc:
    ...
    best_acc = np.savetxt(join('Best','stud_acc({0}).txt').format(model.count_params()) , np.array([candidate]))
    copyfile('student.py', os.path.join('Best','student_{0}.py'.format(model.count_params())) )
    model.save( os.path.join('Best','student_model{0}.h5'.format(model.count_params())) )
else:
...
if hp.slc and hp.n_epochs>0:
    ...
    plt.legend()
    plt.savefig(os.path.join('Figures','student_training_performance.png'))
elif hp.slc:
...
def plot_confusion_matrix(cm, title='Confusion matrix', cmap=plt.cm.Blues, labels=[]):
    ...
    plt.savefig(os.path.join('Figures','student_{0}.png'.format(title.lower().replace(' ','_'))))
    plt.close()
...
if hp.pac and hp.snr_cm:
    ...
    plt.savefig(os.path.join('Figures','student_classification_accuracy.png'))
