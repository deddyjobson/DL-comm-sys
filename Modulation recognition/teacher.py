...
model.add(Convolution2D(256, (1, 3), padding='same', activation="relu", name="conv1", kernel_initializer='glorot_uniform'))
model.add(Dropout(hp.dr))
model.add(Convolution2D(256, (1, 3), padding='valid', activation="relu", name="conv1_5", kernel_initializer='glorot_uniform'))
model.add(Dropout(hp.dr))
model.add(ZeroPadding2D((0, 2)))
model.add(Convolution2D(128, (2, 3), padding="same", activation="relu", name="conv2", kernel_initializer='glorot_uniform'))
model.add(Dropout(hp.dr))
model.add(Convolution2D(128, (2, 3), padding="valid", activation="relu", name="conv3", kernel_initializer='glorot_uniform'))
model.add(Dropout(hp.dr))
model.add(Flatten())
model.add(Dense(256*2, activation='relu', kernel_initializer='he_normal', name="dense1"))
model.add(Dropout(hp.dr))
model.add(Dense(256, activation='relu', kernel_initializer='he_normal', name="dense2"))
model.add(Dropout(hp.dr))
...
filename = os.path.join('Models','teacher_{0}.wts.h5'.format(model.count_params()))
...
if hp.slc and hp.n_epochs>0:
    ...
    plt.legend()
    plt.savefig(os.path.join('Figures','teacher_training_performance.png'))
elif hp.slc:
...
def plot_confusion_matrix(cm, title='Confusion matrix', cmap=plt.cm.Blues, labels=[]):
    ...
    plt.savefig(os.path.join('Figures','teacher_{0}.png'.format(title.lower().replace(' ','_'))))
    plt.close()
...
if hp.pac and hp.snr_cm:
    ...
    plt.savefig(os.path.join('Figures','teacher_classification_accuracy.png'))
