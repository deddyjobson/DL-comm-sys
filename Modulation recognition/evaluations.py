...
parser.add_argument('--num_params',type=int,default=2830427) # number of parameters == unique key word
...
def to_onehot(yy):
    ...
    return yy1
Y_test = to_onehot(list(map(lambda x: mods.index(lbl[x][0]), range(0,n_examples))))
...
model =  keras.models.load_model( os.path.join('Best','model{0}.h5'.format(model.count_params())) )
