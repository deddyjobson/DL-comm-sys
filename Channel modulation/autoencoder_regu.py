...
parser.add_argument('--decay',type=float,default=3e-4) # weight decay adam
...
parser.add_argument('--boundaries',type=int,default=0) # to visualize encodings
...
if not hyper.load_best:
    ...
    try:
        best_regu_acc = np.loadtxt(join('Best','best_regu_acc({1},{2})_{0:.2f}.txt').format(hyper.SNR,hyper.n,hyper.k))
    except OSError:
        best_regu_acc = 0

    if candidate > best_regu_acc:
        print('New best accuracy!')
        np.savetxt(join('Best','best_regu_acc({1},{2})_{0:.2f}.txt').format(hyper.SNR,hyper.n,hyper.k) , np.array([candidate]))
        copyfile('autoencoder_regu.py', join('Best','best_regu_autoencoder({1},{2})_{0}.py'.format(hyper.SNR,hyper.n,hyper.k)) )
        torch.save(encoder, join('Best','best_regu_encoder({1},{2})_{0}.pt'.format(hyper.SNR,hyper.n,hyper.k)))
        torch.save(decoder, join('Best','best_regu_decoder({1},{2})_{0}.pt'.format(hyper.SNR,hyper.n,hyper.k)))
    else:
        print('Too bad, Best accuracy is {0:.2f}%'.format(best_regu_acc))
