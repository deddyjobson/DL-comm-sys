...
parser.add_argument('--T',type=float,default=0.5) # temperature
...
def channel_output(enc):
    return enc + torch.randn_like(enc,device=device)**2 / scaler
...
if not hp.tt:
    try:
        teach_enc = torch.load( join('Best CM Noise','teacher_encoder({1},{2})_{0}.pt'.format(hp.teach_dB,hp.n,hp.k)) )
        teach_dec = torch.load( join('Best CM Noise','teacher_decoder({1},{2})_{0}.pt'.format(hp.teach_dB,hp.n,hp.k)) )
        teach_enc.to(device)
        teach_dec.to(device)
...
if hp.tt:
    ...
    try:
        best_acc = np.loadtxt(join('Best CM Noise','best_teach_acc({1},{2})_{0:.2f}.txt').format(hp.SNR_dB,hp.n,hp.k))
    except OSError:
        best_acc = 0

    if candidate > best_acc:
        print('New best accuracy!')
        np.savetxt(join('Best CM Noise','best_teach_acc({1},{2})_{0:.2f}.txt').format(hp.SNR_dB,hp.n,hp.k) , np.array([candidate]))
        copyfile('channel_modulation_chi2noise.py', join('Best CM Noise','best_channel_modulation_chi2noise({1},{2})_{0}.py'.format(hp.SNR_dB,hp.n,hp.k)) )
        torch.save(teach_enc, join('Best CM Noise','teacher_encoder({1},{2})_{0}.pt'.format(hp.SNR_dB,hp.n,hp.k)))
        torch.save(teach_dec, join('Best CM Noise','teacher_decoder({1},{2})_{0}.pt'.format(hp.SNR_dB,hp.n,hp.k)))
    else:
...
if hp.tsa:
    ...
    try:
        best_acc = np.loadtxt(join('Best CM Noise','best_stud_alone_acc({1},{2})_{0:.2f}.txt').format(hp.SNR_dB,hp.n,hp.k))
    except OSError:
        best_acc = 0

    if candidate > best_acc:
        print('New best accuracy!')
        np.savetxt(join('Best CM Noise','best_stud_alone_acc({1},{2})_{0:.2f}.txt').format(hp.SNR_dB,hp.n,hp.k) , np.array([candidate]))
        torch.save(stud_enc, join('Best CM Noise','stud_alone_encoder({1},{2})_{0}.pt'.format(hp.SNR_dB,hp.n,hp.k)))
        torch.save(stud_dec, join('Best CM Noise','stud_alone_decoder({1},{2})_{0}.pt'.format(hp.SNR_dB,hp.n,hp.k)))
    else:
        print('Too bad, Best accuracy is {0:.2f}%'.format(best_acc))
...
candidate = acc.cpu().detach().numpy()
try:
    best_acc = np.loadtxt(join('Best CM Noise','best_acc({1},{2})_{0:.2f}.txt').format(hp.SNR_dB,hp.n,hp.k))
except OSError:
    best_acc = 0

if candidate > best_acc:
    print('New best accuracy!')
    np.savetxt(join('Best CM Noise','best_acc({1},{2})_{0:.2f}.txt').format(hp.SNR_dB,hp.n,hp.k) , np.array([candidate]))
    copyfile('channel_modulation.py', join('Best CM Noise','best_channel_modulation({1},{2})_{0}.py'.format(hp.SNR_dB,hp.n,hp.k)) )
    torch.save(stud_enc, join('Best CM Noise','student_encoder({1},{2})_{0}.pt'.format(hp.SNR_dB,hp.n,hp.k)))
    torch.save(stud_dec, join('Best CM Noise','student_decoder({1},{2})_{0}.pt'.format(hp.SNR_dB,hp.n,hp.k)))
else:
    print('Too bad, Best accuracy is {0:.2f}%'.format(best_acc))
