...
if hyper.load_best:
    ...
    for _ in range(hyper.depth):
        hiddens.append(MaskedLinear(hyper.M, hyper.M))
        hiddens.append(torch.nn.ReLU())

    encoder = torch.nn.Sequential(
        *hiddens,
        MaskedLinear(hyper.M, hyper.n),
    ...
    decoder = torch.nn.Sequential(
        MaskedLinear(hyper.n, hyper.M), torch.nn.ReLU(),
        *hiddens,
        MaskedLinear(hyper.M, hyper.M)
    )
...
if not hyper.load_best:
    ...
    if candidate > best_acc:
        ...
        copyfile('channel_modulation_masked.py', join('Best','best_channel_modulation_masked({1},{2})_{0}.py'.format(hyper.SNR,hyper.n,hyper.k)) )
